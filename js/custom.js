(function (factory) {
    ...
    const girlsOnlineContainer = document.querySelector('.chat__bottom');
    const chatGirlsContainer = document.querySelector('.chat__girls');
    const chatContainer = document.querySelector('.container__chat');
    const statusContainer = document.querySelector('.container__status');
    const sidebarRight = document.querySelector('.container__sidebar-right');
    const container = document.querySelector('.container');
    const leftSideRestore = document.querySelector('.status__restore-left-side-js');
    const avatarStatus = document.querySelector('.status__avatar-js');
    const backToChat = document.querySelector('.girl__back-btn-js');
    const backToProfile = document.querySelector('.girl__gallery-back-btn-js');
    const input = document.querySelector('.send-message__input');
    const sendBtn = document.querySelector('.send-message__send-btn');
    const popup = document.querySelector('.modal-js');
    const body = document.querySelector('.body');
    const popupCloseBtns = document.querySelectorAll('.js-modal-close-button');
    const galleryPopup = document.querySelector('.modal-gallery-js');
    ...
    function shuffle(sourceArray) {
      ...
      return sourceArray;
    } // Добавление девушки в рабочий чат


    const addGirlOnTheChatContainer = e => {
      const id = e.currentTarget.dataset.id;
      const girl = girlsRandomVideo.find(el => {
        if (el.id == id) {
          return el;
        }
      });
    ...
    } // Возвращаем данные девочки по id из БД


    const girlFindById = (id, bd) => {
      return bd.find(el => {
        if (el.id == id) {
          return el;
        }
    ...
    const messageQuestion = (girl, message, index) => {
      const smile = document.querySelector('#answer');
      const shared = document.querySelector('#shared');
      ...
      !lastMessage(girl.message[chatActive.dataset.countMessage]) && element.querySelector('.video-chat__container').remove();
      messageSum++;
      ...
      if (message.includes('video-call')) {
        ...
        if (message.includes('img')) {
          ...
          p.textContent = message;
          messageContainer.append(p);
          chatGirl.querySelector('.avatar__message-js').classList.remove('avatar__nomessage');
          chatGirl.querySelector('.avatar__message-js').classList.add('avatar__message');
          chatGirl.querySelector('.avatar__message-js').textContent = message;
        }
      }
      ...
      if (index == 0) {
        ...
        element.querySelector('.message__answer').style.margin = '-10px auto 0 0';
        chatActive.querySelectorAll('.read__time-container-js').forEach(time => time.remove());
        element.querySelector('.typing__img-js').src = 'img/content/1x1.png';
        element.querySelector('.read__time-js').textContent = date.getHours() + ':' + (date.getMinutes() < 10 ? '0' : '') + date.getMinutes();
        element.querySelector('.typing__img-mobil-js').src = girl.avatar;
      }
    ...
    const messageVideo = (girl, id, mes, icon) => {
      videoTalking = true;
      const chatActive = chatContainer.querySelector('[data-chatid = "' + girl.id + '"]');
      chatActive.querySelector('.start-conversation') && chatActive.querySelector('.start-conversation').remove();
      girl.message[chatActive.dataset.countMessage].length - 1;
      ...
      if (mes) {
        ...
        if (icon == 'hand') {
          element.querySelector('.message__img-js').src = 'img/content/hand.png';
        } else if (icon == 'face') {
          element.querySelector('.message__img-js').src = 'img/content/sayhi.png';
        }
      ...
      girl.message[chatActive.dataset.countMessage].forEach((message, index) => {
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            chatContainerAddMessage(messageQuestion(girl, message, index), girl.id);
          }, getRandomArbitrary(1, 3) * 1000);
        }, 4000 * (index + 1));
      }); // }, 1000)
    }; // Старт разговора после нажатия кнопки в conversation блоке
    ...
    const startTalking = (id, mes, icon) => {
      ...
      if (!videoTalking) {
        addGirlOnTheChat(girlsRandomVideo[9], false, true);
        messageVideo(girlsRandomVideo[9]);
      }

      changeTextChatGirl(id, true, mes);
      setTimeout(() => {
        chatActive.querySelector('.message__read-icon-js').classList.add('message__read-icon_read');
        const messages = answers[getRandomArbitrary(0, answers.length)];
        let lastMes = messages.length - 1;
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              if (idx == lastMes) {
                ...
                chatContainerAddMessage(messageAnswer(id, message, idx, true), id);
              } else {
                changeTextChatGirl(id, true, message);
                chatContainerAddMessage(messageAnswer(id, message, idx), id);
              }
            }, getRandomArbitrary(1, 3) * 1000);
          }, 4000 * (idx + 1));
        });
    ...
    const sidebarRightToggle = () => sidebarRight.classList.toggle('container__sidebar-right_mobil'); // Слушатели ------------------------------------------------------------------------------


    backToProfile.addEventListener('click', closePopup);
    backToChat.addEventListener('click', sidebarRightToggle);
    avatarStatus.addEventListener('click', sidebarRightToggle);
    profileOpenBtn.addEventListener('click', sidebarRightToggle); // Кнопка в мобильных устройствах, вернуться к списку собеседников
    ...
    leftSideRestore.addEventListener('click', () => {
      ...
      if (window.innerWidth < 700) {
        ...
        document.querySelector('.container__chat').classList.toggle('container__chat_close');
    ...
    sendBtn.addEventListener('click', () => {
      ...
      if (chatActive.querySelector('.start-conversation')) {
        ...
        if (!checkFinalChat(id)) {
          ...
          if (check) {
            chatActive.dataset.countMessage = parseInt(chatActive.dataset.countMessage) + 1;
          }
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              setTimeout(() => {
                chatActive.querySelector('.typing-js').classList.remove('typing_active');
                check ? messageVideo(girl, girl.id) : chatContainerAddMessage(girl.id, girl.id);
    ...
    const conversationListener = (el, id, second = false) => {
      el.querySelector('.start-conversation__smile-btn-js').addEventListener('click', () => {
        !second && startTalking(id, 'You: 😉', 'face');

        if (second) {
          let girl = girlsRandomVideo.find(gi => gi.id === id);
          messageVideo(girl, id, 'You: 😉', 'face');
        }

        sizeDisplay();
      });
      el.querySelector('.start-conversation__gift-btn-js').addEventListener('click', e => modal(e));
      el.querySelector('.start-conversation__sayhi-btn-js').addEventListener('click', () => {
        !second && startTalking(id, 'You: 👋', 'hand');

        if (second) {
          let girl = girlsRandomVideo.find(gi => gi.id === id);
          messageVideo(girl, id, 'You: 👋', 'hand');
        }
    ...
    const addCorrespondenceContainer = (girl, check = false, third = false, second = false) => {
      const chat = document.querySelector('#chat').content;
      const element = chat.cloneNode(true);
      element.querySelector('.container__chat-correspondence').dataset.countMessage = 0;

      if (third) {
        changeTextChatGirl(girl.id, false, '');
        element.querySelector('.start-conversation__start-template').style.display = 'none';
        element.querySelector('.start-conversation__sign-up').classList.add('start-conversation__sign-up_active');
      ...
      element.querySelector('.container__chat-correspondence').dataset.chatid = girl.id;
      check && element.querySelector('.container__chat-correspondence').classList.add('container__chat-correspondence_check-js');
      element.querySelector('.start-conversation__title-js').textContent = girl.name.split(' ')[0];
      element.querySelector('.start-conversation__photo-js').src = girl.avatar;
      element.querySelector('.typing__img-js').src = girl.avatar;
      element.querySelector('.typing__name-js').textContent = girl.name.split(' ')[0]; // changeInfoSideBarRight(girl);

      conversationListener(element, girl.id, second);
      chatContainer.prepend(element);
    }; // Смена данных в правом сайдбаре


    const changeInfoSideBarRight = girl => {
      sidebarRight.querySelector('.avatar-img-js').src = girl.avatar;
      sidebarRight.querySelector('.girl__photo-count-js').textContent = girl.photo.length + ' photo';
      sidebarRight.querySelector('.girl__info-name-js').textContent = girl.name;
    };

    const addListener = girl => {
      chatGirlsContainer.querySelector('[data-id = "' + girl.id + '"]').addEventListener('click', () => {
    ...
    const activeChat = id => {
      ...
      const chatActive = chatContainer.querySelector('[data-chatid = "' + id + '"]');
      girlActive.classList.add('chat__girl_active');
      girlActive.querySelector('.message__count-js').classList.remove('message__count_on');
      chatActive.classList.add('container__chat-correspondence_active');
    }; // Отключаем активный чат


    const removeActiveChat = () => {
      const girls = chatGirlsContainer.querySelectorAll('.chat__girl_active');
      girls.forEach(girl => {
        girl.classList.remove('chat__girl_active');
        chatContainer.querySelector('[data-chatid = "' + girl.dataset.id + '"]').classList.remove('container__chat-correspondence_active');
      });
    }; // Добавление в собеседеники (девушка, открыть окно чата при добавлении, счетчик не прочитанных сообщений)


    const addGirlOnTheChat = (girl, activechat = true, countMes = false, third = false, second = false) => {
      countOnTheChat++;
      const girlElement = document.querySelector('#girl-chat').content;
      const element = girlElement.cloneNode(true);
      element.querySelector('.chat__girl').dataset.id = girl.id; // element.querySelector('.chat__girl').dataset.countMessage = 0;
      ...
      if (activechat) {
        changeInfoSideBarRight(girl);
        addCorrespondenceContainer(girl, true, third, second);
        activeChat(girl.id);
      }
    ...
    const addGirlOnlineContainer = girl => {
      const girlElement = document.querySelector('#girl-online').content;
      const element = girlElement.cloneNode(true);
      element.querySelector('.chat__online').dataset.id = girl.id;
      element.querySelector('.avatar__name').textContent = girl.name;
      element.querySelector('.avatar-img-js').src = girl.avatar;
      girlsOnlineContainer.append(element);
      countGirlOnline++;
    }; // Заполнение девушками блока находящихся в онлайне


    const girlsOnlineFilling = girlsBd => {
      girlsBd.forEach((girl, idx) => {
        if (idx < 5) {
          addGirlOnlineContainer(girl);
        } else return;
      });
    }; //  Навешиваем слушателя на девушек в онлайне


    const addEventList = () => girlsOnlineContainer.querySelectorAll('.chat-online-js').forEach(element => element.addEventListener('click', addGirlOnTheChatContainer)); // Смена текста под аватаркой


    const changeTextChatGirl = (id, text = true, message) => {
      let girl = chatGirlsContainer.querySelector('[data-id = "' + id + '"]');

      if (text) {
        girl.querySelector('.avatar__message-js').classList.remove('avatar__nomessage');
        girl.querySelector('.avatar__message-js').classList.add('avatar__message');
        girl.querySelector('.avatar__message-js').textContent = message;
      } else {
        girl.querySelector('.avatar__message-js').classList.add('avatar__nomessage');
        girl.querySelector('.avatar__message-js').classList.add('avatar__video-icon');
        girl.querySelector('.avatar__message-js').classList.remove('avatar__message');
        girl.querySelector('.avatar__message-js').textContent = 'Invite you to video chat';
      }
    };
    ...
    const modal = e => {
      const modalTitle = popup.querySelector('.modal__title-js');
      const id = e.currentTarget.dataset.id;
      ...
      switch (id) {
        case 'signup':
          modalTitle.textContent = 'One-click sign up';
          break;

        case 'file':
          modalTitle.textContent = 'Sign up to add file';
          break;

        case 'book':
          modalTitle.textContent = 'Sign up to book call';
          break;

        case 'gift':
          modalTitle.textContent = 'Sign up to send gift';
          break;

        case 'video':
          modalTitle.textContent = 'Sign up to make video call';
          break;
    ...
    const gallery = id => {
      galleryPopup.classList.toggle('modal_opened');
      body.classList.toggle('body_fixed');
    };
    Swiper$1.use([Navigation$1]);
    photoGallery.addEventListener('click', e => {
      const id = chatGirlsContainer.querySelector('.chat__girl_active').dataset.id;
      let girl = girlFindById(id, girls);

      if (girl == undefined) {
        girl = girlFindById(id, girl2);
      }

      const liItems = modalGalleryList.querySelectorAll('.modal__gallery-item');
      liItems.forEach(item => item.remove());
      girl.photo.forEach(item => {
        const photo = document.querySelector('#photo');
        const element = photo.content.cloneNode(true);
        element.querySelector('.modal__gallery-img').src = item;
        element.querySelector('.modal__gallery-img').alt = girl.name;
        modalGalleryList.append(element);
      });
      new Swiper$1('.modal__gallery', {
        slidesPerView: 1,
        modules: [Navigation$1],
        navigation: {
          nextEl: '.modal__right-btn-js',
          prevEl: '.modal__left-btn-js'
        }
      });
      gallery();
    });
    ...
    girlsOnlineFilling(girlsRandomVideo);
    addEventList();
